// Demo_Play_Counter : Description : Use to play sounds on demo scene
...
{
	public class Demo_Play_Counter : MonoBehaviour
	{
		...
		public float TimeBetweenTwoSamples;                 // Time between to sample is played
		public AudioClip End_Sample;                // this sound is played when variable Counter = 0
		public float EndSample_Volume = .2f;                //		
		public float TimeBetweenLastSample = .2f;               // Time to wait before playing the last sound
		...
		{                                                           // -> Init
			...
			ImageColor = GetComponent<Image>();                                 // Access UI.Image Component	
			tmp_Counter_2 = Counter;                                                // Init counter

			obj_Manager = GameObject.Find("Demo_Manager").GetComponent<Demo_Manager>(); // Find Demo_Manager gameObject

			txt_Info = GameObject.Find("TXT_Info_Name").GetComponent<Text>();   // Access UI.Text component
			int tmp_NumberOfSample = 0;                                     // Use for demo to know the name of the sample used

			if (Samples.Length > 0) tmp_NumberOfSample += Samples.Length;               // Init parameters to play sounds
		...
		void Update()
		{
			...
			{                                                                       // Use when all the sound is played and you want to restart playing this sounds
				Timer_TimeToLoop = Mathf.MoveTowards(Timer_TimeToLoop, TimeToLoop, Time.deltaTime);
			...
			{                                                           // Use to wait time before playing a new sounds
				Timer_TimeBetweenTwoSamples = Mathf.MoveTowards(Timer_TimeBetweenTwoSamples, TimeBetweenTwoSamples, Time.deltaTime);
				if (Timer_TimeBetweenTwoSamples == TimeBetweenTwoSamples)
				{
					b_TimeBetweenTwoSamples = false;
					Timer_TimeBetweenTwoSamples = 0;
					PlayAList();
			...
			{                                                           // Use to wait time before playing the last sound
				Timer_TimeBetweenLastSample = Mathf.MoveTowards(Timer_TimeBetweenLastSample, TimeBetweenLastSample, Time.deltaTime);
		...
		{                                                   // -> Use to play the final sound
			...
			if (End_Sample)
			{
				s_Audio.volume = EndSample_Volume;
		...
		{                                   // Use when you press a button on demo scene
